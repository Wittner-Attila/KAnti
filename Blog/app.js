import { setupDB, getUsers, getUser, postUser, changeUser, deleteUser, getBlogs, getBlog, postBlog, changeBlog, deleteBlog } from "./util/database.js";
...
app.get("/users/search/:query", (req, res) => {
    ...
    if (usersSearch.length === 0) {
        query.split(" ").forEach(word => {
            if (word.length >= 3) {
                var result = getUsers().filter(user => 
...
app.get("/blogs/search/:query", (req, res) => {
    ...
    if (blogsSearch.length === 0) {
        query.split(" ").forEach(word => {
            if (word.length >= 3) {
                var result = getBlogs().filter(blog => 
...
app.get("/blogs/date/:date", (req, res) => {
    const blogs = getBlogs().filter(blog => new Date(blog.createdAt).toISOString().split('T')[0] === req.params.date);
    if (blogs.length === 0) {
